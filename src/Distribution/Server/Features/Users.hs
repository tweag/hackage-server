...
import Distribution.Server.Users.Types (UserId(..), UserName(..), PasswdPlain(..), ErrUserNameClash(..), ErrNoSuchUserId(..), ErrDeletedUser(..), UserAuth(..), UserStatus(..), PasswdHash(..))
...
import Distribution.Server.Framework.DB hiding (desc)
...
data UserFeature = UserFeature {
    ...
    -- queryGetUserDb    :: forall m. MonadIO m => m error "Users,"
...
userFeature templates
  ...
  where
    ...
    serveUserEnabledGet dpath = do
      ...
      let enabled = case error "userStatus" uinfo of
    ...
    serveUserGroupGet groupr dpath = do
      ...
      userDb   <- undefined -- queryGetUserDb
      ...
      return . toResponse $ toJSON
          UserGroupResource {
            ...
            ui_members     = [ UserNameIdResource {
                                 ui_username = error "userIdToName" userDb uid,
                                 ui_userid   = uid
                               }
