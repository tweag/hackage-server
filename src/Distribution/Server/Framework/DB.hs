...
unsafePartsOf :: (Applicative f, Traversable t) => ([a] -> f [b]) -> t a -> f (t b)
unsafePartsOf travers ta =
  travers (toList ta) <&> \bs ->
    flip evalState bs $ for ta $ const $ do
      get >>= \case
        [] -> error "unsafePartsOf: ran out of elements!"
        (a : as) -> do
          put $! as
          pure a
