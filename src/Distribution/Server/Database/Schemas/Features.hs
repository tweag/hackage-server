...
module Distribution.Server.Database.Schemas.Features
  ...
  , unpDocBuilderReport
  , unpPendingTags
  , unpDependencyForMaintained
  , unpDependencyTriggerBounds
...
instance DBType Version where
  typeInformation =
    let ti = typeInformation @Text
    in ti { encode = contramap (T.pack . prettyShow) $ encode ti
          , decode = fmap forceParseVersion $ decode ti
          }
    where
      forceParseVersion t = case simpleParsec (T.unpack t) of
        Just v  -> v
...
-- | TUF security framework state
--
-- Maps to: Security.State.SecurityState
--
-- PRIMARY KEY (synthetic): ssId
-- There is typically one security state record (or none if not configured).
-- The ID allows for audit trail if needed.
data SecurityStateRow f = SecurityStateRow
  { ssId :: Column f Int64
  , ssSnapshotVersion :: Column f Int32
  , ssTimestampVersion :: Column f Int32
  , ssTimestampTime :: Column f UTCTime
  , ssStateData :: Column f Text  -- serialized SecurityState
  , ssLastUpdated :: Column f UTCTime
  }
  deriving stock (Generic)
  deriving anyclass (Rel8able)

securityStateSchema :: TableSchema (SecurityStateRow Name)
securityStateSchema = TableSchema
  { name = "security_state"
  , columns = SecurityStateRow
      { ssId = "security_state_id"
      , ssSnapshotVersion = "snapshot_version"
      , ssTimestampVersion = "timestamp_version"
      , ssTimestampTime = "timestamp_time"
      , ssStateData = "state_data"
      , ssLastUpdated = "last_updated"
      }
  }
